...
public class JsonQuery
{
    ...
    void Serialization_(SqlSugarClient db)
    {
        var list = db.Queryable<Student>()
            .Where(a => a.SchoolId < 15)
            .ToJson();
    ...
    void Deserialization_(SqlSugarClient db)
    {
        var json = File.ReadAllText(@"D:\SQL_Datas\Untitled-1.json");
        //Key表式运算符: And = 0，  Or = 1

        var conModels = db.Utilities.JsonToConditionalModels(json);

        var dt = db.Queryable<Student>().Where(conModels).ToList();


        for (int i = 0; i < dt.Count; i++)
        {
            Console.WriteLine($"{dt[i].Name}    {dt[i].SchoolId}    ");
        }
